...
const latestPriceData = JSON.parse('{"$id":"BTC_GBP","coin":"BTC","currency":"GBP","updated":1554305869988,"data":{"TOTALVOLUME24HTO":10147125.523436274,"PRICE":3818.12,"CHANGEPCT24HOUR":4.734003741558176}}');
...
describe('modals/PriceCharts', () => {
  ...
  function storeInit(custom) {
    ...
    Wallet.insert({ data: [bitcoinWalletData, erc20WalletData] });
    Coin.insert({ data: [bitcoinData, erc20CoinData] });
    Latest.insert({ data: latestPriceData });
  ...
  describe('loadData()', () => {
    ...
    it('catches any errors', (done) => {
      backEndServiceMock.loadCoinPriceData.mockImplementationOnce(() => { throw new Error('Test Error'); });
      wrapper.vm.loadData();
      setTimeout(() => {
    ...
    it('price data is hidden if no price data exists', (done) => {
      expect(wrapper.contains('.modal-layout-wrapper')).toBeTruthy();
      router.push({ path: '/wallet/single/prices/4' });
      setTimeout(() => {
        expect(wrapper.contains('.modal-layout-wrapper')).toBeFalsy();
        done();
      }, 0);
    });
