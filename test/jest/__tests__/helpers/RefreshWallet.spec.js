...
describe('RefreshWallet', () => {
  ...
  it('returns false if an invalid wallet is passed', async () => {
    const result = await RefreshWallet(mockCoinSDK, { sdk: 'BTC' }, 1);
  ...
  it('can refresh a bitcoin wallet with transaction history', async () => {
    ...
    expect(Wallet.all()[2].confirmedBalance).toBe(2);
