<template>
  <aside
    :class="{ full: full, empty: empty, valid: valid, invalid: !valid }"
    :data-value="value"
    :data-layout="layout"
    class="vue-keyboard"
    role="application">
    <div
      v-for="row in buttons"
      :data-keys="row.length"
      :key="row.length"
      role="row"
      class="vue-keyboard-row">
      <button
        v-for="btn in row"
        :key="btn.id"
        :class="btn.type"
...
export default {
  ...
  computed: {
     ...
     */
    lines() {
      const layout = (Array.isArray(this.layouts)
        ? this.layouts
    ...
    buttons() {
      return this.lines.map((line) => {
        ...
        const stream = line.match(new RegExp(
          `(${Tokens.OPEN_BRACE}|${Tokens.CLOSE_BRACE}|${Tokens.PIPE}|.)`,
          'g',
        ));
        ...
        stream.forEach((char) => {
          if (char === '{') {
            ...
            const text = command.length > 1 ? command[0] : '';
            const action = command.length > 1 ? command[1] : command[0];
            const args = command.length > 2 ? command[2] : null;
            let method = null;

            if (
              ['append', 'backspace', 'space', 'clear', 'goto'].indexOf(action) >= 0
            ) {
              method = this[action].bind(this, args);
