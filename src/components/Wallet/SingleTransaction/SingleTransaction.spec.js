...
describe('SingleTransaction.vue', () => {
  ...
  it('render q-collapsible with a negative amount', () => {
  ...
  it('render q-collapsible with a positive amount', () => {

    const dataCopy = Object.assign({}, propsData.data);
    const stub = {
      qcollapsible: "<qcollapsible-stub label=\"14 October\" sublabel=\"+ 6.22\" class=\"positive-amount\">"
    };
    dataCopy.amount = 6.22;
    wrapper.setProps({data: dataCopy});
    let amount = wrapper.vm.$props.data.amount;
    expect(amount > 0).toBe(true);
    expect(wrapper.html()).toContain(stub.qcollapsible);
  });
