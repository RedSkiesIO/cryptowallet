...
class ToasterFactory {
  ...
  create(status, msg, code, position = 'bottom', stackTrace = null, debug = []) {
    switch (status) {
      case 5:
        this.vm.$emit('WARN', new Toaster(new Toast(msg, msg, 'warning', position, stackTrace, debug)));
        break;
      case 10:
        this.vm.$emit('ERROR', new Toaster(new Toast(msg, msg, 'negative', position, stackTrace, debug)));
        break;
      case 20:
        this.vm.$emit('FAILURE', new Toaster(new Toast(msg, msg, 'negative', position, stackTrace, debug)));
        break;
      case 420:
        this.vm.$emit('GENERAL', new Toaster(new Toast(msg, msg, 'info', position)));
        break;
      case 0:
        this.vm.$emit('SUCCESS', new Toaster(new Toast(msg, msg, 'positive', position)));
        break;
      case 418:
