...
describe('Authed.vue', () => {
  ...
  it('refresher updates the balances of all imported wallets on /wallet', (done) => {
    ...
    wrapper.vm.refresher(() => {
      expect(refreshWallet).toHaveBeenCalledTimes(3);
      setOnline = false;
      done();
    });
  });

  it('cancels refresh if device is offline', (done) => {
    wrapper.vm.refresher(() => {
      expect(backEndServiceMock.loadPriceFeed).toHaveBeenCalledTimes(0);
      setOnline = true;
      done();
    });
  });
  ...
  it('updateBalances function handles errors', (done) => {
    const refreshWallet = jest.fn().mockImplementation(() => { throw new Error('Update balance Error'); });

    wrapper.vm.$walletWorker = {
      refreshWallet,
    };
    wrapper.vm.refresher(() => {
